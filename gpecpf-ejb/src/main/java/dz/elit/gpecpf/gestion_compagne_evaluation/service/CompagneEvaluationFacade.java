...
public class CompagneEvaluationFacade extends AbstractFacade<Compagneevaluation> {
	...
	public boolean checkOutdatedCompagnes() {
		Query q = em.createNamedQuery("Compagneevaluation.findByActive");
		List<Compagneevaluation> list = q.getResultList();
		boolean x = false;
		for (Compagneevaluation compagne : list) {
	...
	public boolean activeCompagne() {
		Query q = em.createNamedQuery("Compagneevaluation.findByActive");
		List<Compagneevaluation> list = q.getResultList();
		return !list.isEmpty() && list.size() >= 1;
	}
	
	public Compagneevaluation currentCompagne() {
		Query q = em.createNamedQuery("Compagneevaluation.findByActive");
		List<Compagneevaluation> list = q.getResultList();
		if (list.isEmpty()) {
			return null;
